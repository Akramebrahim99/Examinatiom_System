...
class StudentController extends Controller
{
    ...
    public function getexam($courseId)
    {
        ...
        foreach ($students as $student) {
            if ($student->id == session('user_id')) {
                $stdInCor= $student->pivot;
            }
        }

        if (now()->greaterThanOrEqualTo((Carbon::parse($course->date_of_exam))->addHours($course->duration)) || now()->lessThan(Carbon::parse($course->date_of_exam)) || $stdInCor->no_of_std_submit >= $course ->no_of_submit) {
            return view('404');
        }
        $count = 0;
    ...
    public function correectexam(Request $request)
    {
        ...
        $stdInCor->course_degree = $studentDegree;
        $stdInCor->no_of_std_submit++;
        $stdInCor->save();
